document.addEventListener("DOMContentLoaded", ()=>{
    ...
    formInputs.forEach(({ inputName, type, options })=>{
        ...
        if (type === "select") options.forEach((option)=>input.add(new Option(option)));
    ...
    const validators = {
        name: (value)=>value.trim().length >= 4 || "Name must be at least 4 characters long",
        age: (value)=>!isNaN(value) && value >= 18 && value <= 90 || "Age must be between 18 and 90",
